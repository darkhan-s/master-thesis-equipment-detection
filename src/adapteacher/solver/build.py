...
def get_default_optimizer_params(
    ...
    Example:
    ::
        torch.optim.SGD(get_default_optimizer_params(cfg, model, weight_decay_norm=0),
    ...
    #@ darkhan-s for regularizing gradients (continual learning)
    ...
    if len(classesAdded)>0:
        for layer_name, value in model.named_parameters(): 
            if "box_predictor.bbox_pred." in layer_name or "box_predictor.cls_score." in layer_name:
                lr = cfg.SOLVER.BASE_LR * cfg.SOLVER.BASE_CLASSIFIER_LR_FACTOR
                print(f'{layer_name} gradient will be readjusted to {lr}')
                hyperparams = copy.copy(defaults)
                hyperparams["lr"] = lr
            
                hyperparams.update(overrides.get(layer_name, {}))
                params.append({"params": [value], **hyperparams})
...
def build_lr_scheduler(
    ...
    ## @darkhan-s cosine scheduler
    elif name == "LRMultiplier":
        return LRMultiplier(
            optimizer,
