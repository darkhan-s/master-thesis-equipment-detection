...
As can be derived from the \nameref{objective}, the detection accuracy on the source dataset is irrelevant in this thesis. The primary goal is to evaluate the performance for the real images. Therefore, for this work, the source dataset is not divided into the training and testing subsets. On the other hand, the target dataset will be split in 85\%/15\%  proportion. The distribution of classes by instances is presented in Figure \ref{tless_distribution_real} for both subsets.   
